...
| `env.AI_API_KEY` | Token cho local LLM / OpenAI-compatible API (dùng cho tool `explain_symbol`). Để trống → fallback về metadata thô | `sk-xxx` hoặc để `""` |
| `env.AI_API_BASE_URL` | Endpoint OpenAI-compatible. Nếu dùng OpenAI thật → bỏ trống | `http://localhost:11434/v1` (Ollama), `http://localhost:1234/v1` (LM Studio) |
| `env.AI_MODEL` | Tên model dùng cho explain_symbol | `qwen2.5-coder:7b`, `llama3`, `gpt-4o-mini`... |
...
```bash
# Ollama
AI_API_KEY=ollama AI_API_BASE_URL=http://localhost:11434/v1 AI_MODEL=qwen2.5-coder:7b \
  npm test -- tests/live-llm.test.ts

# LM Studio
AI_API_KEY=lm-studio AI_API_BASE_URL=http://localhost:1234/v1 AI_MODEL=your-model \
  npm test -- tests/live-llm.test.ts
```
